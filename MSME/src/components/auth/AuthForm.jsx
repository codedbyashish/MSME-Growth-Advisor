    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {mode === 'signup' && (
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={formConfig.fullNamePlaceholder || 'Rajesh Sharma'}
          ...
          <div>
            <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider text-slate-500 dark:text-slate-400">
              {formConfig.businessNameLabel || 'Business Name'}
            </label>
            <div className="relative">
              ...
              <input
                ...
                onChange={handleChange}
                placeholder={formConfig.businessNamePlaceholder || 'Surat Textiles Pvt Ltd'}
                className={`w-full pl-10 pr-4 py-2.5 rounded-xl text-xs font-medium border focus:outline-none focus:ring-2 focus:ring-emerald-400 transition-all ${
                  isDark ? 'bg-slate-950/80 border-slate-800 text-white placeholder:text-slate-600' : 'bg-slate-50 border-slate-300 text-slate-900 placeholder:text-slate-400'
                }`}
              />
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          {mode === 'login' && formConfig.forgotPasswordText && (
      ...
      {/* MSME Category selector for Signup */}
      {mode === 'signup' && categories && (
        <div>
          <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider text-slate-500 dark:text-slate-400">
            {formConfig.categoryLabel || 'MSME Category'}
          </label>
          <select
            name="industry"
            value={formData.industry}
            onChange={handleChange}
            className={`w-full px-3.5 py-2.5 rounded-xl text-xs font-medium border focus:outline-none focus:ring-2 focus:ring-emerald-400 transition-all ${
      ...
      <div className="flex items-center justify-between text-xs pt-1">
        {mode === 'login' ? (
          ...
          <label className="flex items-center space-x-2 cursor-pointer select-none">
            ...
            />
            <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
              I agree to <a href="#terms" className="text-emerald-500 underline">Terms</a> & <a href="#privacy" className="text-emerald-500 underline">Privacy Policy</a>
            </span>
          </label>
        )}
      </div>
